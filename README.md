...
- **六個維度結果**：
  ...
  - ② 職業熱情傾向（含 12 個適合職業推薦）
  - ③ 溝通風格
  - ④ **能量地圖**：充電模式（待越久越有電）vs 耗電模式（待越久越倦怠）
  - ⑤ **場景速覽**：團體中 / 壓力下 / 學習新東西時 / 面對衝突時 的反應模式
  - ⑥ 盲點 / 補位建議：依最弱碼給出實際提醒
...
- **三層人格描述**：修改 `LAYERS` 物件（每碼 × 5 段（trait / career / comm / charge / drain）× 3 層 = 90 條短句）
- **場景描述**：修改 `SCENARIOS` 陣列（4 場景 × 6 碼 × 2 種口吻 = 48 條）
- **職業列表**：修改 `JOB_LISTS`
